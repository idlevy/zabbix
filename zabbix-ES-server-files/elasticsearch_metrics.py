...
def node_fs_stats():
    result = es.nodes.stats ( node_id='_local' , metric='fs' )
    availble = result['nodes'][node_id]['fs']['total']['available_in_bytes']
...
def node_index_stats(metric):
    ...
    if metric == 'total_fs_available_in_bytes':
        node_stats = es.nodes.stats ( node_id='_local' , metric='fs' )
        size = node_stats['nodes'][node_id]['fs']['total']['available_in_bytes']

    if metric == 'os_free_mem':
        node_stats = es.nodes.stats ( node_id='_local' , metric='os' )
        size = node_stats['nodes'][node_id]['os']['mem']['free_in_bytes']
    print size




def node_info(metric):
    node_stats = es.nodes.stats ( node_id='_local' , metric='indices' )
    node_id = node_stats['nodes'].keys ()[0]

    if metric == 'total_fs_available_in_bytes':
        node_stats = es.nodes.stats ( node_id='_local' , metric='fs' )
        result = node_stats['nodes'][node_id]['fs']['total']['available_in_bytes']

    if metric == 'os_free_mem':
        node_stats = es.nodes.stats ( node_id='_local' , metric='os' )
        result = node_stats['nodes'][node_id]['os']['mem']['free_in_bytes']
    if metric == 'os_free_mem_percent':
        node_stats = es.nodes.stats ( node_id='_local' , metric='os' )
        result = node_stats['nodes'][node_id]['os']['mem']['free_percent']
...
node_checks = {'heap_pool_young_gen_mem': node_mem_stats ,
               ...
               'os_free_mem': node_info ,
               'os_free_mem_percent': node_info ,
               'total_fs_available_in_bytes': node_info}
